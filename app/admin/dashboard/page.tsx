...
export default function AdminDashboard() {
  ...
  const [stuckActionInvNum, setStuckActionInvNum] = useState<string | null>(
    null,
  );
  const [stuckActionType, setStuckActionType] = useState<
    "release" | "refund" | null
  >(null);
  ...
  // ── 7b. Suspend user ─────────────────────────────────────────────────────────
  const submitSuspend = async (userId: number) => {
    setUserActionMsg("");
    setUserActionErr("");
    ...
    try {
      ...
      setUserActionMsg(res.data.message ?? "User suspended successfully.");
      setSuspendFormUserId(null);
      setSuspendReason("");
